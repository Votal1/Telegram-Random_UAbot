...
@dp.message_handler(commands=['gruz200'])
...
async def sacrifice(message):
    try:
        ...
        if datetime.now().day != int(r.hget(message.from_user.id, 'time2')) \
            ...
            markup = InlineKeyboardMarkup()
            await message.reply('Після смерті русака, у всіх русаків в цьому чаті (що можуть увійти в топ) впаде '
...
async def donate_shop(message):
    if message.chat.type == 'private':
        ...
        await message.answer('\U0001F31F Погони російських генералів: ' +
                             ...
                             'фото Хеві з ТФ2, слов`янина Рікардо (Увага! Ці фото зникнуть в момент вибору класу), '
                             'або на преміум фото свого класу(Кадиров, Обеме, Горшок, Тесак, Захарченко, Дерек Шовін, '
                             'Янукович, Petya, Джонні Сінс).\n\U0001F943 Настоянка глоду - буст для новачків. Якщо в '
                             'русака менше 400 сили і 5 інтелекту, то настоянка моментально додасть 400 сили і 4 '
