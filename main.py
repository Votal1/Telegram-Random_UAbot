...
async def classes_2(message):
    msg = 'Класи русаків:\n\n\n' \
          ...
          'Далекобійник \U0001F695\U0001F695 - жінки на 50 гривень дешевші. +1% ресурсів за роботу в клані за кожен ' \
          'невідкритий пакунок (максимум 40%).\n\n\n' \
            ...
            if int(r.hget(message.from_user.id, 'clan_time')) != datetime.now().day:
                ...
                if base == 1:
                    ...
                    if int(r.hget(c, 'sawmill')) == 0 and int(r.hget(c, 'mine')) == 0 and int(r.hget(c, 'craft')) == 0:
                        ...
                        packs = int(r.hget(message.from_user.id, 'packs'))
                        trucker = 1
                        if int(r.hget(message.from_user.id, 'class')) in (32, 33) and packs > 0:
                            if packs > 40:
                                packs = 40
                            trucker += packs * 0.01
                        if int(r.hget(c, 'camp')) == 1:
