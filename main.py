...
async def classes(message):
    msg = 'Класи русаків:\n\n\n' \
          ...
          '\U0001F4B5 500 грн. Збільшує силу в дуелях на 40% (замість 10%), якщо більше ніж 90 здоров`я. 2% шанс ' \
...
async def classes_3(message):
    msg = 'Класи русаків:\n\n\n' \
          ...
          ' (якщо в героя є АК-47). Також з шансом 10% в бою герой і ворог можуть невелике поранення ' \
        ...
        if call.data.startswith('clan_side'):
            ...
            if str(call.from_user.id).encode() in r.smembers('cl' + cid):
                if call.data.startswith('clan_shop_1'):
                    ...
                    await bot.edit_message_text(msg, cid, call.message.message_id, reply_markup=markup)
