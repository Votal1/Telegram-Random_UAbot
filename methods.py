...
def c_shop(c, page):
    ...
    if page == 1:
        ...
        if int(r.hget(c, 'build5')) == 1:
            ...
            msg += '\n\U0001F9EA Цукор [Допомога, міцність=1] - збільшує силу при годуванні на 15 (до 5000 сили) або' \
...
def auto_clan_settings(c):
    ...
    if int(r.hget(c, 'allow')) == 0:
        msg += '\n\n\U0001F4E5 В клан може приєднатись КОЖЕН бажаючий.'
    else:
        msg += '\n\n\U0001F4E5 В клан можна приєднатись тільки З ДОЗВОЛУ адміністраторів.'
    if int(r.hget(c, 'war_allow')) == 0:
        msg += '\n\n\U0001F3DF В міжчатову битву може зайти КОЖЕН бажаючий.'
    elif int(r.hget(c, 'war_allow')) == 1:
        msg += '\n\n\U0001F3DF В міжчатову битву в перші 10 хвилин може зайти тільки УЧАСНИК КЛАНУ.'
    else:
        msg += '\n\n\U0001F3DF В міжчатову битву в можуть зайти тільки УЧАСНИКИ КЛАНУ.'
    if int(r.hget(c, 'salary')) == 0:
        msg += '\n\n\U0001F4B5 За роботу НЕ видається зарплата з кланових ресурсів.'
    else:
        msg += '\n\n\U0001F4B5 За роботу з рахунку клану ЗНІМАТИМЕТЬСЯ 8 гривень: 5 гривень робітнику, 3 - податок.'
    if int(r.hget(c, 'recruitment')) == 0:
        msg += '\n\n\U0001F530 В Соледарі НЕ ВІДКРИТО набір в клан. Щоб відкрити, ' \
               'треба платити по 3 радіотехніки в день.'
    else:
        msg += '\n\n\U0001F530 В Соледарі ВІДКРИТО набір в клан.'
    if int(r.hget(c, 'notification')) == 0:
        msg += '\n\n\U0001F69B Сповіщення про конвой ВИМКНЕНІ. Щоб увімкнути, треба платити по 5 радіотехніки в день.'
    else:
        msg += '\n\n\U0001F69B Сповіщення про конвой УВІМКНЕНІ.'
